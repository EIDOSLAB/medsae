...
def feature_density_diagram(features):
    """
    Compute the wandb table of the log10 latent sparsity (how often it fires) for all latents.
    """
    ...
    table = wandb.Table(data=list(zip(x_vals, y_vals)), columns=["Log10 Sparsity Value", "Density"])
    return table
...
def evaluate(
    ...
    features = t.empty((0, dictionary.dict_size), dtype=t.float32, device=device)
    ...
    if final:
        out["Log10 Latent Sparsity Diagram"] = feature_density_diagram(features)
