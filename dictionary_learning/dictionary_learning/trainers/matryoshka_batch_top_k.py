...
class MatryoshkaBatchTopKTrainer(SAETrainer):
    def __init__(
        ...
        dict_size_modified = dict_size - 16
        group_sizes = [2, 8, 6, int(1/32*dict_size_modified), int(1/16*dict_size_modified), int(1/8*dict_size_modified), int(1/4*dict_size_modified)]
    ...
    def update_threshold(self, f: t.Tensor):
        ...
        with t.autocast(device_type=device_type, enabled=False), t.no_grad():
            ...
            if active.size(0) == 0:
                min_activation = t.tensor(0.0, device=self.device)  # Ensure it's a tensor
            else:
                min_activation = active.min().detach().to(dtype=t.float32)

            if self.ae.threshold is None or self.ae.threshold < 0:
