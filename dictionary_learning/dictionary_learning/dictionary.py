...
class AutoEncoder(Dictionary, nn.Module):
    ...
    def __init__(self, activation_dim, dict_size):
        ...
        # Discover-Then-Name reproduction
        init.kaiming_uniform_(self.encoder.weight, nonlinearity="relu")
        # Bias (approach from nn.Linear)
        fan_in = self.encoder.weight.size(1)
        bound = 1 / math.sqrt(fan_in) if fan_in > 0 else 0
        init.uniform_(self.encoder.bias, -bound, bound)

        init.normal_(self.decoder.weight, mean=0, std=1)
        with t.no_grad():
            t.nn.functional.normalize(self.decoder.weight, dim=-2, out=self.decoder.weight)
    ...
    def from_pretrained(cls, path, dtype=t.float, device=None, normalize_decoder=True):
        ...
        if "encoder._weight" in state_dict:
            state_dict["encoder.weight"] = state_dict["encoder._weight"].squeeze(0)
            state_dict["decoder.weight"] = state_dict["decoder._weight"].squeeze(0)
            state_dict["bias"] = state_dict["tied_bias"].squeeze(0)
            state_dict["encoder.bias"] = state_dict["encoder._bias"].squeeze(0)
            del state_dict['pre_encoder_bias._bias_reference']
            del state_dict['tied_bias']
            del state_dict['post_decoder_bias._bias_reference']
            del state_dict['encoder._weight']
            del state_dict['encoder._bias']
            del state_dict['decoder._weight']
