...
def new_wandb_process(config, log_queue, entity, project):
    ...
    while True:
        try:
            ...
            if log == "DONE":
                break
            if "Log10 Latent Sparsity Diagram" in log:
                    wandb.log({"eval/Log10 Latent Sparsity Distribution" : wandb.plot.line(
                        log["Log10 Latent Sparsity Diagram"],
                        "Log10 Sparsity Value",
                        "Density",
                        title="Log10 Latent Sparsity Distribution",
                    )})
                    log.remove("Log10 Latent Sparsity Diagram")
            wandb.log(log)
        except Empty:
            continue
    wandb.finish()
    ...
    with t.no_grad():
        ...
        for i, trainer in enumerate(trainers):
            ...
            if not transcoder:
                ...
                log[f"train/frac_variance_explained"] = frac_variance_explained.item()
            else:  # transcoder
            ...
            log.update({f"train/{k}": v.cpu().item() if isinstance(v, t.Tensor) else v for k, v in losslog.items()})
            log[f"train/l0"] = l0
            trainer_log = trainer.get_logging_parameters()
            for name, value in trainer_log.items():
                if isinstance(value, t.Tensor):
                    value = value.cpu().item()
                log[f"train/{name}"] = value

            # log evaluation metrics if provided
            if eval_metrics:
                trainer_eval_metrics = eval_metrics[i]
                log.update({f"eval/{k}": v for k, v in trainer_eval_metrics.items()})
...
def trainSAE(
    ...
    if save_dir is not None:
        save_dirs = [
            os.path.join(save_dir, trainer_configs[i]['wandb_name']) for i in range(len(trainer_configs))
        ]
    ...
    for epoch in range(epochs):
        print("Epoch :", epoch+1)
        for act in tqdm(data, total=steps, desc=f"Training ..."):
            if normalize_activations:
                act /= norm_factor
            ...
            if eval_steps is not None and step % eval_steps == 0:
                eval_metrics = []
                for trainer in trainers:
                    eval_metrics.append(evaluate(
                        trainer.ae,  
                        eval_data, 
                        max_len=128,  
                        batch_size=eval_data.out_batch_size,  
                        io="out", 
                ...
                log_stats(
                    trainers, step, act, activations_split_by_head, transcoder, log_queues=log_queues, verbose=verbose, eval_metrics=eval_metrics
                )
        ...
        if save_epochs is not None and epoch in save_epochs:
            for dir, trainer in zip(save_dirs, trainers):
                if dir is not None:
                    ...
                    t.save(
                        checkpoint,
                        os.path.join(dir, "checkpoints", f"ae_{epoch}.pt"),
                    )
